...
export function formatDriverPrompt(options: DriverPromptOptions): string {
  ...
  const keywordsSection = keywords
    ? KEYWORDS_SECTION.replace("{keywords}", keywords)
    : "";

  // Previous results section
  let previousResultsSection: string;
  if (previousResults && batchNum > 1) {
    previousResultsSection = PREVIOUS_RESULTS_SECTION
      .replace("{checked_count}", String(previousResults.checked_count))
